...
- A **crew card** below the map showing who's currently aboard the ISS (live headcount + names), via a small serverless proxy — see [Crew data](#crew-data) below.
...
This means the crew card only shows real data when the site is actually deployed on Vercel (or run locally via `vercel dev` — see below). Under plain `npm run dev`, it shows its "unavailable" fallback state, which is expected, not a bug.
...
`.env.local` is gitignored. The project ships with `DEMO_KEY` as a placeholder so `npm run dev` works out of the box, but you'll hit its rate limit fast while iterating on the gallery.
...
On Vercel, set `VITE_NASA_API_KEY` as a project environment variable — Vite inlines it into the build at build time. `api/crew.ts` needs no env vars of its own.
